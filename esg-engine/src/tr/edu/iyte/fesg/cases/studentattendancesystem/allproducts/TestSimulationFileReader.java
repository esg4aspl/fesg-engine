...
public class TestSimulationFileReader {
	...
	public static void main(String[] args) throws IOException {
		String faultsTestExecutions = "files/Cases/StudentAttendanceSystemPL/1RegFiles/_spl_sas_2_faults-testexecutions-20percent-1234567890";		
		...
		for (int i = 0; i < faultsTestExecutionsFiles.length; i++) {
			...
			if (faultsTestExecutionsFiles[i].isFile()) {
				...
				if (faultsTestExecutionsFileName.contains("length")) {
					...
					row.add(scenarioID);
					String prefix3 = "sas-0123-0045_rrg-1seq-tb_length";
					testSetNumber += faultsTestExecutionsFileName.substring(prefix3.length(), prefix3.length() + 1);
					String prefix4 = "sas-0123-0045_rrg-1seq-tb_length2-";
		...
		try {
			csvWriter = new FileWriter("files/Cases/StudentAttendanceSystemPL/table7 part2" + ".csv");
		} catch (IOException e) {
