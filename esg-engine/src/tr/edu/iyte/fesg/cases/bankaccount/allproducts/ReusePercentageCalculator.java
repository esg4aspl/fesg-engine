...
public class ReusePercentageCalculator {
	...
	public static void main(String[] args) throws IOException {
		String PUCFolder = "files/MXEFiles/BankAccountPL/Results_IncrementalTestSequenceComposition/TestSequences";
		String existingProductFolder = "files/MXEFiles/BankAccountPL/Results_IncrementalTestSequenceComposition/ExistingProductTestSequences";
		String singleModelFolder = "files/MXEFiles/BankAccountPL/Results_SingleModel/TestSequences";
		...
		for (int i = 0; i < puclistOfFiles.length; i++) {
			...
			if (eplistOfFiles[i].isFile()) {
				...
				String prefix1 = "ba-";
				pucID += epCurrentFileName.substring(prefix1.length(), prefix1.length() + 2);

				String prefix2 = "ba-02-";
				scenarioID += epCurrentFileName.substring(prefix2.length(), prefix2.length() + 2);

				String prefix3 = "ba-02-04-";
				epID += epCurrentFileName.substring(prefix3.length(), prefix3.length() + 2);

				String prefix4 = "ba-02-04-01_length";
				covlength += epCurrentFileName.substring(prefix4.length(), prefix4.length() + 1);

				epTestSequences = TestSequenceCoverageAnalyser
						.testCasesFromFile(existingProductFolder + "/" + eplistOfFiles[i].getName());
			...
			if (puclistOfFiles[i].isFile()) {
				...
				String prefix1 = "ba-";
				String pid = pucCurrentFileName.substring(prefix1.length(), prefix1.length() + 2);

				String prefix2 = "ba-02-";
				String sid = pucCurrentFileName.substring(prefix2.length(), prefix2.length() + 2);

				String prefix3 = "ba-02-04-INC_length";
			...
			if (smlistOfFiles[i].isFile()) {
				...
				String prefix1 = "ba-";
				String pid = smCurrentFileName.substring(prefix1.length(), prefix1.length() + 2);

				String prefix2 = "ba-02-";
				String sid = smCurrentFileName.substring(prefix2.length(), prefix2.length() + 2);

				String prefix3 = "ba-02-04_length";
			...
			// System.out.println(pucID + "-" + scenarioID + ": " + commonSequences + "/" +
			// pucTestSequences.size() + " " + percentage);
			rows.add(row);
		...
		try {
			csvWriter = new FileWriter("files/MXEFiles/BankAccountPL/reuse" + ".csv");
		} catch (IOException e) {
		...
		csvWriter.append(",");
		csvWriter.append("Test Sequences Reused in Test Set");
		csvWriter.append(",");
		csvWriter.append("Events Reused in Test Sequences");
		csvWriter.append(",");
